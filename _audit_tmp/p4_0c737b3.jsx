...
export default function Page4Payment({ onNavigate }) {
    ...
    const reportConnectedWallet = useCallback((rawAddress) => {
        ...
        // R6.3 TEMP DEBUG тАФ remove after runtime trace
        ...
        const unsubscribe = tonConnectUI.onModalStateChange((state) => {
            ...
            if (state?.status === "closed" && !tonConnectUI.wallet) {
                ...
                if (localWalletStatus === WALLET_CONNECTION_STATUS.CONNECTING) {
                    ...
                    // R6.3 TEMP DEBUG тАФ remove after runtime trace
                    console.log(
                        "[R6.3 TRACE] modal closed without wallet тЖТ WALLET_DISCONNECT_REPORT",
                        { localWalletStatus }
                    );
    ...
    // R6.11A тАФ Capture SDK Universal Link from the same connector.connect()
    ...
    // R6.7B TEMP DEBUG тАФ observe SDK status only; remove after handshake probe
    useEffect(() => {
