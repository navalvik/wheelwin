...
export function resolveRecordId(recordType, payload, metadata = {}) {
    ...
    switch (recordType) {
        ...
        case TON_FINANCIAL_RECORD_TYPES.DEPOSIT_SESSION:
            return metadata.depositId
                ?? payload?.depositId
                ?? metadata.recordId
                ?? null;

        case TON_FINANCIAL_RECORD_TYPES.DEPLOYMENT_AUTHORIZATION:
            return metadata.authorizationId
        ...
        case TON_FINANCIAL_RECORD_TYPES.RECOVERY_DATA:
            return metadata.recoveryRecordId
...
export function isImmutableRecord(recordType, status = null) {
    ...
    if (recordType === TON_FINANCIAL_RECORD_TYPES.DEPLOYMENT_AUTHORIZATION) {

        return DEPLOYMENT_AUTHORIZATION_TERMINAL_STATUSES.includes(status);
