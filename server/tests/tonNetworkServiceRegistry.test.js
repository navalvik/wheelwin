...
function testSharedSigningMaterialAcrossNetworks() {
    const registry = new TonNetworkServiceRegistry({
        env: {
            TON_TESTNET_ENDPOINT: "https://testnet.example.invalid",
            TON_MAINNET_ENDPOINT: "https://mainnet.example.invalid",
            ...
            TON_DEPLOYER_MNEMONIC: "shared-mnemonic",
            TON_API_KEY: "shared-api-key"
        },
    ...
    assert(
        testnet.deployerMnemonic === "shared-mnemonic",
        "Testnet must use the shared deployer mnemonic"
    );
    assert(
        mainnet.deployerMnemonic === "shared-mnemonic",
        "Mainnet must use the same shared deployer mnemonic"
    );
    assert(
        mainnet.deployerMnemonic === testnet.deployerMnemonic,
        "Testnet and Mainnet must intentionally share the same mnemonic"
    );
    assert(
        mainnet.apiKey === "shared-api-key",
        "Mainnet must use the shared API key when no dedicated key is configured"
    );

    console.log("Shared signing material across networks: passed");
}
...
testSharedSigningMaterialAcrossNetworks();
