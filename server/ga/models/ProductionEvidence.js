/**
 * R9.0A — Immutable production verification evidence.
 */

import { createHash } from "node:crypto";

import { CHECK_STATUS } from "../ProductionConfiguration.js";

/**
 * @param {unknown} value
 */
export function canonicalize(value) {

    if (value === null || typeof value !== "object") {

        return value;

    }

    if (Array.isArray(value)) {

        return value.map((v) => canonicalize(v));

    }

    const out = {};

    for (const key of Object.keys(value).sort()) {

        out[key] = canonicalize(value[key]);

    }

    return out;

}

/**
 * @param {object} payload
 */
export function hashEvidencePayload(payload) {

    return createHash("sha256")
        .update(JSON.stringify(canonicalize(payload)))
        .digest("hex");

}

/**
 * @param {{
 *   id?: string,
 *   verification: string,
 *   status?: string,
 *   timestamp?: number,
 *   durationMs?: number,
 *   details?: object,
 *   recommendations?: string[]
 * }} input
 */
export function createProductionEvidence(input) {

    const statusKey = String(input.status || CHECK_STATUS.FAIL).toUpperCase();

    const status = CHECK_STATUS[statusKey] ?? CHECK_STATUS.FAIL;

    const timestamp = Number.isFinite(input.timestamp)
        ? input.timestamp
        : Date.now();

    const durationMs = Number.isFinite(input.durationMs)
        ? input.durationMs
        : 0;

    const details = Object.freeze({ ...(input.details ?? {}) });

    const recommendations = Object.freeze([...(input.recommendations ?? [])]);

    const verification = String(input.verification);

    const evidenceHash = hashEvidencePayload({
        verification,
        status,
        details,
        recommendations
    });

    const id = input.id
        ? String(input.id)
        : `pev-${verification}-${evidenceHash.slice(0, 12)}`;

    return Object.freeze({
        id,
        verification,
        status,
        timestamp,
        durationMs,
        details,
        recommendations,
        evidenceHash
    });

}
