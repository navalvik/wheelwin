...
/**
 * R6.11B тАФ dump ENTIRE original TonConnect / wallet / callback error object.
...
/** R6.11A/B/C тАФ dump complete SDK / wallet rejection errors + autopsy store. */
function dumpTonConnectError(label, error, context = {}) {
    ...
    // R6.11C тАФ persist forensic error into window.__TONCONNECT_AUTOPSY__
    try {
...
/** R6.11B тАФ print final autopsy report for one handshake attempt. */
...
function resolveMostProbableFailurePoint(report) {
    ...
    if (report.callbackException != null) {
        ...
        return "Custom callback threw (CALLBACK_EXCEPTION) тАФ distinct from SDK error";
...
/** R6.11A тАФ structured wallet field dump for onStatusChange / state traces. */
...
/** R6.11A тАФ summarize connector.connect() wallet source / info args. */
...
export default function Page4Payment({ onNavigate }) {
    ...
    // P6.2 тАФ wallet connection; P6.3 тАФ authoritative Payment Session after READY.
    // P6.7 тАФ Page4 stays open until server OPEN_PAGE5 (never local navigation).
    ...
    // R6.11A тАФ Desktop Connection convenience (presentation only)
    ...
    // R6.11A тАФ chronological handshake attempt trace (diagnostics only)
    ...
    // R6.11B тАФ per-attempt autopsy findings + temporary global handlers
    ...
    const pushHandshakeTrace = useCallback((step, detail = {}) => {
        ...
        // R6.11C тАФ feed unified autopsy timeline (keep TRACE console too)
        try {
    ...
    const installAutopsyGlobalHandlers = useCallback(() => {
        ...
        window.onunhandledrejection = function tonConnectAutopsyUnhandledRejection(
            ...
            // R6.11C тАФ persist into autopsy browserErrors[]
            try {
    ...
    const beginAutopsyAttempt = useCallback((attemptId) => {
        ...
        // R6.11C тАФ start unified window.__TONCONNECT_AUTOPSY__ session
        try {
    ...
    const printAttemptAutopsyReport = useCallback((reason) => {
        ...
        // R6.11C тАФ sync final steps into window.__TONCONNECT_AUTOPSY__
        try {
    ...
    // R6.11B тАФ restore temporary window handlers if Page4 unmounts mid-attempt
    useEffect(() => {
            ...
            runAutopsyGuardedCallback("onModalStateChange", () => {
                ...
                if (state?.status === "closed" && !tonConnectUI.wallet) {
                    ...
                    if (localWalletStatus === WALLET_CONNECTION_STATUS.CONNECTING) {
                        ...
                        console.log(
                            "[TonConnect TRACE] modal closed without wallet тЖТ WALLET_DISCONNECT_REPORT",
                            { localWalletStatus }
                        );
    ...
    // R6.11A/B тАФ Capture SDK Universal Link from the same connector.connect()
    ...
    // R6.11A тАФ probe optional SDK event hooks (CONNECTING/CONNECTED/тАж)
    useEffect(() => {
    ...
    // R6.7B / R6.11A/B тАФ observe SDK status; full wallet dump + raw error autopsy
    useEffect(() => {
            ...
            (wallet) => {
                ...
                runAutopsyGuardedCallback("onStatusChange wallet callback", () => {
                    ...
                    if (!wallet) {
                        ...
                        console.log("[TonConnect TRACE] wallet: null тЖТ disconnected");
    ...
    // R6.11A тАФ Open SDK Universal Link for desktop/browser wallet launch.
    // Approach (mirrors @tonconnect/ui openLinkBlank): window.open with
    // noopener,noreferrer on the exact SDK-generated string тАФ works for
                ...
                <div className="paymentPanel">
                    ...
                    <div className="smartContractStatus">
                        ...
                        <div className="page4__connectActions">
                            ...
                            {/*
                              R6.11A тАФ Desktop Connection block lives on Page4
                              ...
                              tonConnectUI.connector.connect() тАФ same value the
